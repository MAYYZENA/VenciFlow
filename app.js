// Expor funções utilitárias para testes.js
...
// Inicialização global do window.dadosEstoque (CRÍTICO para gráficos)
if (typeof window !== 'undefined' && typeof window.dadosEstoque === 'undefined') {
  window.dadosEstoque = [];
  console.log('✅ window.dadosEstoque inicializado globalmente');
}
...
(function () {
  ...
  async function login() {
    ...
    try {
      ...
      if (typeof verificarStatusTrial === 'function') {
        setTimeout(() => verificarStatusTrial(), 2000);
      }
  ...
  async function salvarProduto() {
    try {
      // Capturar e sanitizar inputs primeiro
      ...
      if (!produtoEditando && nome && marca) {
        const duplicado = await verificarProdutoDuplicado(nome, marca, lote);
        if (duplicado) {
          const confirmar = confirm(
            `⚠️ Produto similar já existe:\n\n` +
            `${duplicado.nome}\n` +
            `Marca: ${duplicado.marca}\n` +
            `${duplicado.lote ? `Lote: ${duplicado.lote}\n` : ''}` +
      ...
      let camposExtras = {};
      if (window.coletarCamposExtras) {
        camposExtras = window.coletarCamposExtras();
      }
      mostrarLoader(true);
      const dadosProduto = {
      ...
      localStorage.removeItem('rascunho-produto');
  ...
  if (typeof window.dadosEstoque === 'undefined') {
    window.dadosEstoque = [];
  }
  
  // ===== PAGINAÇÃO DO ESTOQUE =====
  let ultimoDocEstoque = null;
  let estoqueAcabou = false;
  const TAMANHO_PAGINA_ESTOQUE = 20;
  ...
  async function carregarEstoque(reset = false) {
    try {
      ...
      let query = getCollection('estoque').orderBy('validade', 'asc').limit(TAMANHO_PAGINA_ESTOQUE);
      if (ultimoDocEstoque) {
        query = query.startAfter(ultimoDocEstoque);
      }
      ...
      snap.forEach(doc => {
        ...
        ultimoDocEstoque = doc;
        count++;
      });
      renderTabela(window.dadosEstoque);
      mostrarLoader(false);
      // Mostrar botão "Carregar mais" se ainda houver mais
      const btnMais = document.getElementById('btnCarregarMaisEstoque');
      if (btnMais) {
        btnMais.style.display = estoqueAcabou ? 'none' : 'block';
      }
    } catch (e) { mostrarLoader(false); handleError(e); }
  }

  // Função para resetar e carregar a primeira página
  window.recarregarEstoque = function() {
    carregarEstoque(true);
  }

  // Função para carregar mais
  window.carregarMaisEstoque = function() {
  ...
  function renderTabela(lista) {
    ...
    lista.forEach(p => {
      ...
      const tags = obterTagsProduto(p.id);
      const tagsHTML = tags.size > 0 ? Array.from(tags).map(tag => 
        `<span style="display: inline-block; padding: 2px 8px; background: ${getCorTag(tag)}; color: white; border-radius: 4px; font-size: 11px; font-weight: 600; margin-right: 4px;">${tag}</span>`
      ).join('') : '';
      ...
      <tr class="${rowClass} ${status === 'vencido' ? 'table-danger' : status === 'alerta' ? 'table-warning' : ''}" data-id="${p.id}">
        ...
        <td>
          <div class="btn-group btn-group-sm">
            ...
            <button class="btn btn-outline-secondary" onclick="mostrarGerenciadorTags('${p.id}')" title="Tags">🏷️</button>
            <button class="btn btn-outline-info" onclick="mostrarModalNotas('${p.id}')" title="Notas">📝</button>
            <button class="btn btn-outline-success" onclick="imprimirEtiqueta('${p.id}')" title="Imprimir">🖨️</button>
  ...
  async function editarProduto(id) {
    try {
      ...
      if (window.preencherCamposExtrasEdicao) window.preencherCamposExtrasEdicao(p.camposExtras);
  ...
  const comandos = [
    ...
    { nome: '� Busca Global', acao: () => mostrarBuscaGlobal(), tags: 'pesquisar procurar buscar tudo global search find' },
    { nome: '🎨 Personalizar Dashboard', acao: () => abrirEditorDashboard(), tags: 'customizar widgets layout dashboard' },
    { nome: '🎓 Tour Guiado', acao: () => iniciarTour(), tags: 'tutorial ajuda onboarding guia' },
      ...
      <div style="padding: 24px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 12px; font-size: 14px;">
          <div style="display: flex; align-items: center; gap: 8px;">
            <kbd style="padding: 4px 8px; background: var(--bg-secondary); border: 1px solid var(--border); border-radius: 4px;">Ctrl/Cmd + P</kbd>
          </div>
          <div>Command Palette (todos os comandos)</div>
          
          <div style="display: flex; align-items: center; gap: 8px;">
            <kbd style="padding: 4px 8px; background: var(--bg-secondary); border: 1px solid var(--border); border-radius: 4px;">Ctrl/Cmd + F</kbd>
          </div>
          <div>Busca Global (em tudo)</div>
...
auth.onAuthStateChanged(user => {
  if (user) {
    ...
    setTimeout(async () => {
      // Garantir que window.dadosEstoque existe antes de carregar gráficos
      if (!window.dadosEstoque || !Array.isArray(window.dadosEstoque)) {
        window.dadosEstoque = [];
        console.log('✅ window.dadosEstoque inicializado para gráficos');
      }
...
async function inicializarGraficosDashboard() {
  try {
    console.log('🔄 Iniciando carregamento de gráficos do dashboard...');
    
    // CRITICAL: Garantir que window.dadosEstoque está inicializado ANTES de qualquer operação
    if (typeof window.dadosEstoque === 'undefined') {
      window.dadosEstoque = [];
      console.log('⚠️ inicializarGraficosDashboard: window.dadosEstoque era undefined - inicializando');
    }
    
    if (!window.dadosEstoque) {
      window.dadosEstoque = [];
      console.log('⚠️ inicializarGraficosDashboard: window.dadosEstoque era null - inicializando');
    }
    
    if (!Array.isArray(window.dadosEstoque)) {
      console.error('❌ inicializarGraficosDashboard: window.dadosEstoque não é array - reinicializando');
      window.dadosEstoque = [];
    }
    
    console.log('✅ inicializarGraficosDashboard: window.dadosEstoque verificado -', window.dadosEstoque.length, 'produtos');
    
    // Se estiver vazio, carregar dados primeiro
    if (window.dadosEstoque.length === 0) {
      console.log('⚠️ window.dadosEstoque vazio - carregando do Firestore...');
      if (typeof carregarEstoque === 'function') {
        await carregarEstoque();
        // Aguardar carregamento
        await new Promise(resolve => setTimeout(resolve, 1000));
        console.log('✅ Estoque carregado:', window.dadosEstoque.length, 'produtos');
      } else {
        console.error('❌ Função carregarEstoque não existe!');
        return;
      }
    }
    
    // Verificar novamente se tem dados após carregamento
    if (window.dadosEstoque.length === 0) {
      console.log('⚠️ Nenhum produto no estoque - gráficos não serão exibidos');
      return;
    }

    
    // Só carregar gráficos se o canvas existir
    ...
    if (document.getElementById('graficoMarcas')) {
...
async function carregarGraficoEvolucao() {
  ...
  // CRITICAL: Garantir que window.dadosEstoque existe ANTES de qualquer operação
  if (typeof window.dadosEstoque === 'undefined') {
    window.dadosEstoque = [];
    console.warn('⚠️ carregarGraficoEvolucao: window.dadosEstoque era undefined - inicializando');
  }
  
  if (!window.dadosEstoque) {
    window.dadosEstoque = [];
    console.warn('⚠️ carregarGraficoEvolucao: window.dadosEstoque era null - inicializando');
  }
  
  if (!Array.isArray(window.dadosEstoque)) {
    console.error('❌ carregarGraficoEvolucao: window.dadosEstoque não é array - reinicializando');
    window.dadosEstoque = [];
  }
  
  console.log('📊 carregarGraficoEvolucao: window.dadosEstoque tem', window.dadosEstoque.length, 'produtos');
  
  // Se estiver vazio, carregar dados do Firestore primeiro
  if (window.dadosEstoque.length === 0) {
    console.log('⚠️ window.dadosEstoque vazio - tentando carregar do Firestore...');
    try {
      if (typeof carregarEstoque === 'function') {
        await carregarEstoque();
        // Aguardar carregamento
    ...
    // Se ainda estiver vazio, não desenhar gráfico
    if (window.dadosEstoque.length === 0) {
      console.log('⚠️ Sem dados para gráfico de evolução após tentativa de carregamento');
      return;
    }
  }
...
async function carregarGraficoStatus() {
  ...
  // CRITICAL: Garantir que window.dadosEstoque existe
  if (typeof window.dadosEstoque === 'undefined') {
    window.dadosEstoque = [];
    console.warn('⚠️ carregarGraficoStatus: window.dadosEstoque era undefined - inicializando');
  }
  
  if (!window.dadosEstoque) {
    window.dadosEstoque = [];
    console.warn('⚠️ carregarGraficoStatus: window.dadosEstoque era null - inicializando');
  }
  
  if (!Array.isArray(window.dadosEstoque)) {
    console.error('❌ carregarGraficoStatus: window.dadosEstoque não é array - reinicializando');
    window.dadosEstoque = [];
  }
  
  // Se estiver vazio, carregar do Firestore
  if (window.dadosEstoque.length === 0) {
    console.log('⚠️ window.dadosEstoque vazio - tentando carregar...');
    try {
      if (typeof carregarEstoque === 'function') {
        await carregarEstoque();
        await new Promise(resolve => setTimeout(resolve, 500));
    ...
    if (window.dadosEstoque.length === 0) {
      console.log('⚠️ Sem dados para gráfico de status');
      return;
    }
  }
...
async function carregarGraficoTopProdutos() {
  ...
  if (typeof window.dadosEstoque === 'undefined') {
    window.dadosEstoque = [];
    console.warn('⚠️ carregarGraficoTopProdutos: window.dadosEstoque era undefined - inicializando');
  }
  
  if (!window.dadosEstoque) {
    window.dadosEstoque = [];
    console.warn('⚠️ carregarGraficoTopProdutos: window.dadosEstoque era null - inicializando');
  }
  
  if (!Array.isArray(window.dadosEstoque)) {
    console.error('❌ carregarGraficoTopProdutos: window.dadosEstoque não é array - reinicializando');
    window.dadosEstoque = [];
  }
  ...
  if (window.dadosEstoque.length === 0) {
    ...
    if (window.dadosEstoque.length === 0) {
      console.log('⚠️ Sem dados para gráfico top produtos');
      return;
    }
  }
...
async function carregarGraficoMarcas() {
  ...
  if (typeof window.dadosEstoque === 'undefined') {
    window.dadosEstoque = [];
    console.warn('⚠️ carregarGraficoMarcas: window.dadosEstoque era undefined - inicializando');
  }
  
  if (!window.dadosEstoque) {
    window.dadosEstoque = [];
    console.warn('⚠️ carregarGraficoMarcas: window.dadosEstoque era null - inicializando');
  }
  
  if (!Array.isArray(window.dadosEstoque)) {
    console.error('❌ carregarGraficoMarcas: window.dadosEstoque não é array - reinicializando');
    window.dadosEstoque = [];
  }
  ...
  if (window.dadosEstoque.length === 0) {
    ...
    if (window.dadosEstoque.length === 0) {
      console.log('⚠️ Sem dados para gráfico de marcas');
      return;
    }
  }
...
async function carregarHistorico() {
  ...
  const TAMANHO_PAGINA_HISTORICO = 20;

  window._historicoPaginacao = window._historicoPaginacao || { ultimoDoc: null, acabou: false, carregado: false };

  async function carregarHistoricoPaginado(reset = false) {
    try {
      mostrarLoader(true);
      const tabela = document.getElementById('tabelaHistorico');
      if (!tabela) { mostrarLoader(false); return; }
      if (reset || !window._historicoPaginacao.carregado) {
        tabela.innerHTML = '';
        window._historicoPaginacao.ultimoDoc = null;
        window._historicoPaginacao.acabou = false;
        window._historicoPaginacao.carregado = true;
      }
      if (window._historicoPaginacao.acabou) {
        mostrarLoader(false);
        return;
      }
      let query = getCollection('historico').orderBy('timestamp', 'desc').limit(TAMANHO_PAGINA_HISTORICO);
      if (window._historicoPaginacao.ultimoDoc) {
        query = query.startAfter(window._historicoPaginacao.ultimoDoc);
      }
      const snap = await query.get();
      if (snap.empty) {
        window._historicoPaginacao.acabou = true;
        mostrarLoader(false);
        return;
      }
      snap.forEach(doc => {
        ...
        window._historicoPaginacao.ultimoDoc = doc;
      });
      // Mostrar botão "Carregar mais" se ainda houver mais
      const btnMais = document.getElementById('btnCarregarMaisHistorico');
      if (btnMais) {
        btnMais.style.display = window._historicoPaginacao.acabou ? 'none' : 'block';
      }
      mostrarLoader(false);
    } catch (e) {
      mostrarLoader(false); handleError(e);
    }
  }

  // Função para resetar e carregar a primeira página
  window.recarregarHistorico = function() {
    window._historicoPaginacao.carregado = false;
    carregarHistoricoPaginado(true);
  }

  // Função para carregar mais
  window.carregarMaisHistorico = function() {
    carregarHistoricoPaginado(false);
  }

  // Inicialização padrão

  window.recarregarHistorico();
...
// ==================== VERIFICAÇÃO AVANÇADA DE LIMITE ====================
async function verificarLimiteUsoPorPlano() {
  try {
    if (!empresaAtual?.plano) return false;
    ...
    const limite = limites[empresaAtual.plano] || 50;
    const percentual = (totalProdutos / limite) * 100;
    
    // Alerta em 80%
    if (percentual >= 80 && percentual < 90) {
      mostrarToast(`⚠️ ${Math.round(percentual)}% do limite usado (${totalProdutos}/${limite})`, 'warning');
    }
    
    // Alerta em 90% com notificação persistente
    if (percentual >= 90 && percentual < 100) {
      const upgradeMsg = `🚨 ATENÇÃO: ${totalProdutos}/${limite} produtos!\n\nVocê está próximo do limite. Considere fazer upgrade.`;
      mostrarToast(upgradeMsg, 'warning');
      
      if ('Notification' in window && Notification.permission === 'granted') {
        new Notification('⚠️ Limite Próximo', {
          body: `${totalProdutos}/${limite} produtos (${Math.round(percentual)}%)`,
    ...
    // Bloquear em 100%
    if (totalProdutos >= limite) {
      mostrarToast(`🚫 Limite de ${limite} produtos atingido! Upgrade necessário.`, 'error');
      return true;
    }
...
async function verificarProdutoDuplicado(nome, marca, lote) {
  try {
    const query = await getCollection('estoque')
      .where('nome', '==', nome)
      .where('marca', '==', marca)
      .get();
    ...
    // Se tem lote, verificar lote também
    if (lote) {
      const duplicado = query.docs.find(doc => doc.data().lote === lote);
      return duplicado ? { id: duplicado.id, ...duplicado.data() } : null;
    }
...
function calcularDiasVencimento(dataValidade) {
  ...
  const hoje = new Date();
  const validade = dataValidade instanceof Date ? dataValidade : 
                   (dataValidade.toDate ? dataValidade.toDate() : new Date(dataValidade));
...
let dragDropAtivo = false;

function inicializarDragDrop() {
  const zonas = ['body', 'estoque', 'dashboard'];
  
  zonas.forEach(zonaId => {
    const zona = zonaId === 'body' ? document.body : document.getElementById(zonaId);
    if (!zona) return;
    ...
    zona.addEventListener('dragover', (e) => {
      ...
      if (!dragDropAtivo && auth.currentUser) {
        mostrarOverlayDragDrop();
        dragDropAtivo = true;
      }
    });
    ...
    zona.addEventListener('dragleave', (e) => {
      ...
      // Só remove se saiu do body
      if (e.target === document.body) {
        esconderOverlayDragDrop();
        dragDropAtivo = false;
      }
    });
    ...
    zona.addEventListener('drop', (e) => {
      ...
      if (arquivos.length > 0) {
        processarArquivosDrop(arquivos);
      }
    });
  });
}

function mostrarOverlayDragDrop() {
    ...
    overlay.innerHTML = `
      ...
      ">
        <div style="text-align: center; color: white;">
          ...
          <h2 style="font-size: 36px; font-weight: 700; margin: 0 0 16px 0;">Solte o arquivo aqui!</h2>
          <p style="font-size: 18px; opacity: 0.9; margin: 0;">Importação automática de Excel/CSV</p>
          <div style="margin-top: 32px; display: flex; gap: 20px; justify-content: center; font-size: 14px; opacity: 0.8;">
            <div>✅ .xlsx</div>
            <div>✅ .xls</div>
            <div>✅ .csv</div>
          </div>
...
function esconderOverlayDragDrop() {
...
async function processarArquivosDrop(arquivos) {
  for (let i = 0; i < arquivos.length; i++) {
    const arquivo = arquivos[i];
    const nome = arquivo.name.toLowerCase();
    
    if (nome.endsWith('.xlsx') || nome.endsWith('.xls') || nome.endsWith('.csv')) {
      mostrarToast(`📥 Importando: ${arquivo.name}`, 'info');
      
      // Usar a função de importação existente
      ...
      reader.onload = async (e) => {
        try {
          ...
          for (const linha of json) {
            try {
              const produto = {
                codigo: linha['Código'] || linha['Codigo'] || linha['código'] || '',
                nome: linha['Produto'] || linha['Nome'] || linha['produto'] || '',
                marca: linha['Marca'] || linha['marca'] || 'Sem marca',
                categoria: linha['Categoria'] || linha['categoria'] || 'Geral',
                lote: linha['Lote'] || linha['lote'] || '',
                quantidade: Number(linha['Quantidade'] || linha['quantidade'] || 0),
                estoqueMinimo: Number(linha['Estoque Mínimo'] || linha['estoqueMinimo'] || 0),
                validade: linha['Validade'] || linha['validade'] || null
              };
              
              if (!produto.nome || produto.quantidade <= 0) {
                erros++;
                continue;
              }
              ...
              await getCollection('estoque').add({
                ...produto,
                validade: produto.validade ? firebase.firestore.Timestamp.fromDate(new Date(produto.validade)) : null,
          ...
          mostrarToast(`✅ ${sucesso} produtos importados! ${erros > 0 ? `(${erros} erros)` : ''}`, 'success');
          carregarEstoque();
          registrarAcao('importar', { arquivo: arquivo.name, sucesso, erros });
      ...
      mostrarToast(`⚠️ Formato não suportado: ${arquivo.name}`, 'warning');
    }
  }
}
...
let tagsDisponiveis = new Set();
const coresTags = [
  '#e74c3c', '#3498db', '#2ecc71', '#f39c12', '#9b59b6',
  '#1abc9c', '#e67e22', '#34495e', '#f1c40f', '#95a5a6'
];

// Carregar tags salvas
try {
  const savedTags = localStorage.getItem('tags-disponiveis');
  if (savedTags) {
    tagsDisponiveis = new Set(JSON.parse(savedTags));
  } else {
    // Tags padrão
    tagsDisponiveis = new Set(['Promoção', 'Premium', 'Novo', 'Importado', 'Orgânico']);
    localStorage.setItem('tags-disponiveis', JSON.stringify(Array.from(tagsDisponiveis)));
  }
} catch (e) {}

function mostrarGerenciadorTags(produtoId = null) {
  const produtoTags = produtoId ? obterTagsProduto(produtoId) : new Set();
  ...
  const html = `
    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 9999; padding: 20px;" onclick="if(event.target === this) this.remove()">
      <div style="background: var(--bg-card); border-radius: 16px; width: 100%; max-width: 600px; max-height: 80vh; overflow: hidden; box-shadow: var(--shadow-lg);" onclick="event.stopPropagation()">
        ...
        <div style="padding: 24px; border-bottom: 1px solid var(--border);">
          <h3 style="margin: 0; color: var(--text-primary); font-size: 20px; font-weight: 600;">
            🏷️ ${produtoId ? 'Gerenciar Tags do Produto' : 'Todas as Tags'}
          </h3>
          <p style="margin: 8px 0 0 0; color: var(--text-secondary); font-size: 14px;">
            ${produtoId ? 'Selecione as tags para este produto' : 'Criar e gerenciar tags do sistema'}
          </p>
        </div>
        ...
        <div style="padding: 20px; overflow-y: auto; max-height: calc(80vh - 200px);">
          ...
          ${produtoId ? `
            <div style="margin-bottom: 24px;">
              <label style="display: block; margin-bottom: 12px; font-weight: 600; color: var(--text-primary);">Tags do Produto:</label>
              <div style="display: flex; flex-wrap: wrap; gap: 8px; min-height: 40px; padding: 12px; background: var(--bg-secondary); border-radius: 8px;">
                ${produtoTags.size === 0 ? '<span style="color: var(--text-secondary); font-size: 13px;">Nenhuma tag selecionada</span>' : 
                  Array.from(produtoTags).map(tag => `
                    <span style="display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px; background: ${getCorTag(tag)}; color: white; border-radius: 6px; font-size: 13px; font-weight: 500;">
                      ${tag}
                      <button onclick="removerTagProduto('${produtoId}', '${tag}'); this.closest('div[style*=\\'position: fixed\\']').remove(); mostrarGerenciadorTags('${produtoId}');" style="background: transparent; border: none; color: white; cursor: pointer; font-size: 16px; line-height: 1; padding: 0;">×</button>
                    </span>
                  `).join('')
                }
              </div>
          ...
          <div style="margin-bottom: 16px;">
            <label style="display: block; margin-bottom: 8px; font-weight: 600; color: var(--text-primary);">
              ${produtoId ? 'Adicionar Tag:' : 'Nova Tag:'}
            </label>
            <div style="display: flex; gap: 8px;">
              <input 
                type="text" 
                ...
                onkeypress="if(event.key === 'Enter') { ${produtoId ? `adicionarTagProduto('${produtoId}', this.value)` : 'criarNovaTag(this.value)'}; this.value = ''; }"
                style="flex: 1; padding: 10px; border: 1px solid var(--border); border-radius: 6px; background: var(--bg-main); color: var(--text-primary);"
              />
              <button onclick="const input = document.getElementById('novaTagInput'); ${produtoId ? `adicionarTagProduto('${produtoId}', input.value)` : 'criarNovaTag(input.value)'}; input.value = '';" class="btn btn-primary">
                ➕ ${produtoId ? 'Adicionar' : 'Criar'}
              </button>
            </div>
          </div>
          ...
          <div>
            <label style="display: block; margin-bottom: 12px; font-weight: 600; color: var(--text-primary);">
              ${produtoId ? 'Tags Disponíveis:' : 'Tags Existentes:'}
            </label>
            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
              ${Array.from(tagsDisponiveis).map(tag => `
                <button 
                  onclick="${produtoId ? `adicionarTagProduto('${produtoId}', '${tag}')` : `if(confirm('Excluir tag \\'${tag}\\'?')) excluirTag('${tag}')`}"
                  style="display: inline-flex; align-items: center; gap: 6px; padding: 8px 14px; background: ${getCorTag(tag)}; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 500; transition: transform 0.2s;"
                  onmouseover="this.style.transform = 'scale(1.05)'"
                  onmouseout="this.style.transform = 'scale(1)'"
                  title="${produtoId ? 'Clique para adicionar' : 'Clique para excluir'}"
                >
                  ${produtoId && produtoTags.has(tag) ? '✓ ' : ''}${tag}
                </button>
              `).join('')}
            </div>
          </div>
        ...
        <div style="padding: 16px 24px; border-top: 1px solid var(--border); display: flex; justify-content: flex-end;">
          <button onclick="this.closest('div[style*=\\'position: fixed\\']').remove(); ${produtoId ? 'carregarEstoque();' : ''}" class="btn btn-primary">
            Fechar
          </button>
        </div>
...
window.mostrarGerenciadorTags = mostrarGerenciadorTags;

function getCorTag(tag) {
  const hash = Array.from(tag).reduce((acc, char) => acc + char.charCodeAt(0), 0);
  return coresTags[hash % coresTags.length];
}

function criarNovaTag(nome) {
  const tag = nome.trim();
  if (!tag) return;
  
  if (tagsDisponiveis.has(tag)) {
    mostrarToast('⚠️ Tag já existe', 'warning');
    return;
  }
  ...
  mostrarToast(`✅ Tag "${tag}" criada!`, 'success');
...
function excluirTag(tag) {
  tagsDisponiveis.delete(tag);
  localStorage.setItem('tags-disponiveis', JSON.stringify(Array.from(tagsDisponiveis)));
  mostrarToast(`🗑️ Tag "${tag}" excluída`, 'success');
...
function obterTagsProduto(produtoId) {
  try {
    const tags = localStorage.getItem(`produto-tags-${produtoId}`);
    return tags ? new Set(JSON.parse(tags)) : new Set();
  } catch (e) {
    return new Set();
  }
}

function adicionarTagProduto(produtoId, tag) {
  const tagNome = tag.trim();
  if (!tagNome) return;
  
  // Adicionar às tags disponíveis se não existir
  if (!tagsDisponiveis.has(tagNome)) {
    tagsDisponiveis.add(tagNome);
    localStorage.setItem('tags-disponiveis', JSON.stringify(Array.from(tagsDisponiveis)));
  }
  
  const tags = obterTagsProduto(produtoId);
  tags.add(tagNome);
  
  localStorage.setItem(`produto-tags-${produtoId}`, JSON.stringify(Array.from(tags)));
  mostrarToast(`🏷️ Tag adicionada!`, 'success');
  
  // Reabrir modal
  document.querySelector('div[style*="position: fixed"]')?.remove();
  mostrarGerenciadorTags(produtoId);
}
window.adicionarTagProduto = adicionarTagProduto;

function removerTagProduto(produtoId, tag) {
  const tags = obterTagsProduto(produtoId);
  tags.delete(tag);
  
  localStorage.setItem(`produto-tags-${produtoId}`, JSON.stringify(Array.from(tags)));
  mostrarToast(`🗑️ Tag removida`, 'success');
}
window.removerTagProduto = removerTagProduto;

// ================ SISTEMA DE NOTAS E COMENTÁRIOS ================

function mostrarModalNotas(produtoId) {
  const notas = obterNotasProduto(produtoId);
  const produto = Array.from(produtos.values()).find(p => p.id === produtoId);
  
  const html = `
    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 9999; padding: 20px;" onclick="if(event.target === this) this.remove()">
      <div style="background: var(--bg-card); border-radius: 16px; width: 100%; max-width: 700px; max-height: 85vh; overflow: hidden; box-shadow: var(--shadow-lg);" onclick="event.stopPropagation()">
        
        <div style="padding: 24px; border-bottom: 1px solid var(--border); background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
          <h3 style="margin: 0; color: white; font-size: 20px; font-weight: 600;">
            📝 Notas: ${produto?.nome || 'Produto'}
          </h3>
          <p style="margin: 8px 0 0 0; color: rgba(255,255,255,0.9); font-size: 14px;">
        ...
        <div style="padding: 20px; overflow-y: auto; max-height: calc(85vh - 240px);">
          
          <!-- Nova Nota -->
          <div style="background: var(--bg-secondary); border-radius: 12px; padding: 16px; margin-bottom: 20px;">
            <textarea 
              id="novaNota"
              placeholder="Digite sua nota aqui... (use @ para mencionar usuários)"
              style="width: 100%; min-height: 80px; padding: 12px; border: 1px solid var(--border); border-radius: 8px; resize: vertical; background: var(--bg-main); color: var(--text-primary); font-family: inherit; font-size: 14px;"
            ></textarea>
            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 12px;">
              <small style="color: var(--text-secondary);">💡 Dica: Markdown suportado</small>
              <button onclick="adicionarNota('${produtoId}')" class="btn btn-primary">
                ➕ Adicionar Nota
              </button>
            </div>
          </div>
          ...
          <div>
            <h4 style="margin: 0 0 16px 0; color: var(--text-primary); font-size: 16px; font-weight: 600;">
              📋 Histórico (${notas.length} nota${notas.length !== 1 ? 's' : ''})
            </h4>
            ...
            ${notas.length === 0 ? `
              <div style="text-align: center; padding: 40px; color: var(--text-secondary);">
                ...
                <small>Seja o primeiro a adicionar uma observação</small>
              </div>
            ` : notas.map((nota, idx) => `
              <div style="background: var(--bg-secondary); border-left: 4px solid ${nota.tipo === 'importante' ? '#e74c3c' : '#3498db'}; border-radius: 8px; padding: 16px; margin-bottom: 12px;">
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                  <div style="display: flex; align-items: center; gap: 8px;">
                    <div style="width: 32px; height: 32px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-weight: 600; font-size: 12px;">
                      ${nota.usuario.substring(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <div style="font-weight: 600; color: var(--text-primary); font-size: 14px;">${nota.usuario}</div>
                      <div style="font-size: 11px; color: var(--text-secondary);">
                        ${new Date(nota.timestamp).toLocaleString('pt-BR')}
                      </div>
                    </div>
                  </div>
                  <button onclick="excluirNota('${produtoId}', ${idx})" style="background: transparent; border: none; color: var(--text-secondary); cursor: pointer; padding: 4px 8px;" title="Excluir">🗑️</button>
                </div>
                <div style="color: var(--text-primary); font-size: 14px; line-height: 1.6; white-space: pre-wrap;">${nota.texto}</div>
                ${nota.editado ? '<div style="font-size: 11px; color: var(--text-secondary); margin-top: 8px; font-style: italic;">✏️ Editado</div>' : ''}
              </div>
            `).join('')}
          </div>
        ...
        <div style="padding: 16px 24px; border-top: 1px solid var(--border); display: flex; justify-content: space-between;">
          <button onclick="exportarNotas('${produtoId}')" class="btn btn-secondary btn-sm">
            📤 Exportar Notas
          </button>
          <button onclick="this.closest('div[style*=\\'position: fixed\\']').remove()" class="btn btn-primary">
            Fechar
...
function obterNotasProduto(produtoId) {
  try {
    const notas = localStorage.getItem(`produto-notas-${produtoId}`);
...
function adicionarNota(produtoId) {
  const textarea = document.getElementById('novaNota');
  const texto = textarea.value.trim();
  
  if (!texto) {
    mostrarToast('⚠️ Digite uma nota primeiro', 'warning');
    return;
  }
  ...
  const nota = {
    ...
    timestamp: Date.now(),
    tipo: texto.toLowerCase().includes('importante') || texto.toLowerCase().includes('urgente') ? 'importante' : 'normal',
    editado: false
  };
  
  notas.unshift(nota); // Adiciona no início
  
  localStorage.setItem(`produto-notas-${produtoId}`, JSON.stringify(notas));
  
  mostrarToast('✅ Nota adicionada!', 'success');
  registrarAcao('adicionar-nota', { produtoId });
  
  // Reabrir modal
  document.querySelector('div[style*="position: fixed"]')?.remove();
  mostrarModalNotas(produtoId);
}
window.adicionarNota = adicionarNota;

function excluirNota(produtoId, index) {
  ...
  mostrarToast('🗑️ Nota excluída', 'success');
...
function exportarNotas(produtoId) {
  const notas = obterNotasProduto(produtoId);
  const produto = Array.from(produtos.values()).find(p => p.id === produtoId);
  
  if (notas.length === 0) {
    mostrarToast('⚠️ Nenhuma nota para exportar', 'warning');
    return;
  }
  
  const texto = `NOTAS - ${produto?.nome || 'Produto'}\n` +
    `Data: ${new Date().toLocaleString('pt-BR')}\n` +
    `Total: ${notas.length} nota(s)\n\n` +
    `${'='.repeat(60)}\n\n` +
    notas.map((nota, idx) => 
      `[${idx + 1}] ${new Date(nota.timestamp).toLocaleString('pt-BR')}\n` +
      `Usuário: ${nota.usuario}\n` +
      `Tipo: ${nota.tipo === 'importante' ? '🔴 IMPORTANTE' : '📝 Normal'}\n` +
      `\n${nota.texto}\n` +
      `${'-'.repeat(60)}\n`
    ).join('\n');
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `Notas_${produto?.nome || 'Produto'}_${Date.now()}.txt`;
  a.click();
  URL.revokeObjectURL(url);
  
  mostrarToast('📥 Notas exportadas!', 'success');
}
window.exportarNotas = exportarNotas;

// ================ IMPRESSÃO DE ETIQUETAS ================

function imprimirEtiqueta(produtoId) {
  const produto = Array.from(produtos.values()).find(p => p.id === produtoId);
  if (!produto) return;
  
  const html = `
    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 9999; padding: 20px;" onclick="if(event.target === this) this.remove()">
      <div style="background: var(--bg-card); border-radius: 16px; width: 100%; max-width: 600px; overflow: hidden; box-shadow: var(--shadow-lg);" onclick="event.stopPropagation()">
        ...
        <div style="padding: 24px;">
          ...
          <!-- Preview da Etiqueta -->
          <div id="etiquetaPreview" style="background: white; border: 2px dashed #ccc; border-radius: 8px; padding: 24px; margin-bottom: 24px; text-align: center; color: #000;">
            <div style="font-size: 18px; font-weight: 700; margin-bottom: 8px;">${produto.nome}</div>
            <div style="font-size: 14px; color: #666; margin-bottom: 12px;">Marca: ${produto.marca}</div>
            <div id="codigoBarras" style="margin: 16px 0;"></div>
            <div style="font-size: 13px; color: #666;">Código: ${produto.codigo || 'N/A'}</div>
            <div style="font-size: 13px; color: #666; margin-top: 8px;">Validade: ${formatDate(produto.validade) || 'N/A'}</div>
            <div style="font-size: 13px; color: #666; margin-top: 8px;">Quantidade: ${produto.quantidade}</div>
          </div>
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">
            <label style="display: flex; align-items: center; gap: 8px; padding: 12px; background: var(--bg-secondary); border-radius: 8px; cursor: pointer;">
              <input type="radio" name="tamanhoEtiqueta" value="pequeno" checked> 
              <span style="color: var(--text-primary);">Pequeno (5x3cm)</span>
            </label>
            <label style="display: flex; align-items: center; gap: 8px; padding: 12px; background: var(--bg-secondary); border-radius: 8px; cursor: pointer;">
              <input type="radio" name="tamanhoEtiqueta" value="medio"> 
              <span style="color: var(--text-primary);">Médio (8x5cm)</span>
            </label>
            <label style="display: flex; align-items: center; gap: 8px; padding: 12px; background: var(--bg-secondary); border-radius: 8px; cursor: pointer;">
              <input type="radio" name="tamanhoEtiqueta" value="grande"> 
              <span style="color: var(--text-primary);">Grande (10x7cm)</span>
            </label>
            <label style="display: flex; align-items: center; gap: 8px; padding: 12px; background: var(--bg-secondary); border-radius: 8px; cursor: pointer;">
              <input type="checkbox" id="incluirQRCode" checked> 
              <span style="color: var(--text-primary);">Incluir QR Code</span>
            </label>
          </div>
          
          <div style="background: var(--bg-secondary); padding: 12px; border-radius: 8px; margin-bottom: 16px;">
            <label style="display: block; margin-bottom: 8px; color: var(--text-primary); font-weight: 600;">Quantidade de cópias:</label>
            <input type="number" id="quantidadeCopias" value="1" min="1" max="100" style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 6px; background: var(--bg-main); color: var(--text-primary);">
          </div>
        ...
        <div style="padding: 16px 24px; border-top: 1px solid var(--border); display: flex; justify-content: space-between;">
          ...
          <button onclick="executarImpressao('${produtoId}')" class="btn btn-primary">
            🖨️ Imprimir
          </button>
        </div>
  ...
  setTimeout(() => {
    if (window.JsBarcode && produto.codigo) {
      try {
        const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        document.getElementById('codigoBarras').appendChild(svg);
        ...
        console.log('Código de barras não disponível');
      }
    }
  }, 100);
}
...
function executarImpressao(produtoId) {
  const copias = parseInt(document.getElementById('quantidadeCopias').value) || 1;
  
  // Clonar o preview para impressão
  ...
  for (let i = 0; i < copias; i++) {
    printWindow.document.write('<div class="etiqueta">');
  ...
  document.querySelector('div[style*="position: fixed"]')?.remove();
  mostrarToast(`🖨️ Imprimindo ${copias} etiqueta(s)...`, 'success');
  registrarAcao('imprimir-etiqueta', { produtoId, copias });
}
...
function criarIndicadorConexao() {
  ...
  atualizarIndicadorConexao(indicador);
  document.body.appendChild(indicador);
  
  indicador.addEventListener('click', () => {
    mostrarDetalhesConexao();
  });
}

function atualizarIndicadorConexao(indicador) {
  if (!indicador) indicador = document.getElementById('statusConexao');
  if (!indicador) return;
  
  if (statusConexao.online) {
    indicador.style.background = 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)';
    indicador.style.color = 'white';
    indicador.innerHTML = '<span style="font-size: 10px;">🟢</span> Online';
  } else {
    indicador.style.background = 'linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)';
    indicador.style.color = 'white';
    indicador.innerHTML = '<span style="font-size: 10px;">🔴</span> Offline';
  }
}

function mostrarDetalhesConexao() {
  const html = `
    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 10001; padding: 20px;" onclick="if(event.target === this) this.remove()">
      <div style="background: var(--bg-card); border-radius: 16px; width: 100%; max-width: 500px; overflow: hidden; box-shadow: var(--shadow-lg);" onclick="event.stopPropagation()">
        
        <div style="padding: 24px; border-bottom: 1px solid var(--border); background: ${statusConexao.online ? 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)' : 'linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)'};">
          <h3 style="margin: 0; color: white; font-size: 20px; font-weight: 600;">
            ${statusConexao.online ? '🟢 Sistema Online' : '🔴 Sistema Offline'}
          </h3>
          <p style="margin: 8px 0 0 0; color: rgba(255,255,255,0.9); font-size: 14px;">
            ${statusConexao.online ? 'Conectado ao Firebase' : 'Trabalhando em modo offline'}
          </p>
        </div>
        ...
        <div style="padding: 24px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
              ...
              <div>
                ...
                <div style="font-size: 18px; font-weight: 700; color: var(--text-primary);">
                  ${statusConexao.online ? '✅ Conectado' : '❌ Desconectado'}
                </div>
              </div>
              
              <div>
                <div style="font-size: 12px; color: var(--text-secondary); margin-bottom: 4px;">Última Verificação</div>
                <div style="font-size: 14px; font-weight: 600; color: var(--text-primary);">
                  ${new Date(statusConexao.ultimaVerificacao).toLocaleTimeString('pt-BR')}
                </div>
              </div>
          ...
          ${!statusConexao.online ? `
            <div style="background: #fff3cd; border: 1px solid #ffc107; border-radius: 8px; padding: 16px; margin-bottom: 16px;">
              <div style="display: flex; align-items: center; gap: 12px; color: #856404;">
                <div style="font-size: 24px;">⚠️</div>
                <div>
                  <div style="font-weight: 600; margin-bottom: 4px;">Modo Offline Ativo</div>
                  <div style="font-size: 13px;">Suas alterações serão sincronizadas quando a conexão for restaurada.</div>
                </div>
              </div>
            </div>
          ` : ''}
          ...
          <div style="background: var(--bg-secondary); border-radius: 8px; padding: 16px;">
            <h4 style="margin: 0 0 12px 0; font-size: 14px; font-weight: 600; color: var(--text-primary);">Funcionalidades Disponíveis:</h4>
            <div style="display: flex; flex-direction: column; gap: 8px; font-size: 13px; color: var(--text-secondary);">
              <div style="display: flex; align-items: center; gap: 8px;">
                <span>${statusConexao.online ? '✅' : '✅'}</span>
                <span>Visualizar produtos em cache</span>
              </div>
              <div style="display: flex; align-items: center; gap: 8px;">
                <span>${statusConexao.online ? '✅' : '⚠️'}</span>
                <span>Adicionar/editar produtos ${!statusConexao.online ? '(local)' : ''}</span>
              </div>
              <div style="display: flex; align-items: center; gap: 8px;">
                <span>${statusConexao.online ? '✅' : '❌'}</span>
        ...
        <div style="padding: 16px 24px; border-top: 1px solid var(--border); display: flex; justify-content: ${statusConexao.online ? 'flex-end' : 'space-between'};">
          ${!statusConexao.online ? `
            <button onclick="tentarReconectar()" class="btn btn-primary btn-sm">
              🔄 Tentar Reconectar
            </button>
          ` : ''}
          <button onclick="this.closest('div[style*=\\'position: fixed\\']').remove()" class="btn btn-secondary">
...
window.mostrarDetalhesConexao = mostrarDetalhesConexao;

async function tentarReconectar() {
  statusConexao.tentativasReconexao++;
  mostrarToast('🔄 Tentando reconectar...', 'info');
  
  try {
    // Tentar fazer uma requisição ao Firebase
    await db.collection('estoque').limit(1).get();
    
    statusConexao.online = true;
    statusConexao.ultimaVerificacao = Date.now();
    atualizarIndicadorConexao();
    
    mostrarToast('✅ Reconectado com sucesso!', 'success');
    
    // Fechar modal
    document.querySelector('div[style*="position: fixed"]')?.remove();
    
  } catch (e) {
    mostrarToast('❌ Falha na reconexão. Tente novamente.', 'error');
  }
}
...
window.addEventListener('online', () => {
  statusConexao.online = true;
  statusConexao.ultimaVerificacao = Date.now();
  statusConexao.tentativasReconexao = 0;
  atualizarIndicadorConexao();
  mostrarToast('🟢 Conexão restaurada!', 'success');
});
...
window.addEventListener('offline', () => {
  ...
  statusConexao.ultimaVerificacao = Date.now();
  atualizarIndicadorConexao();
  mostrarToast('🔴 Você está offline. Modo offline ativado.', 'warning');
});
...
// Criar indicador ao carregar
if (typeof window !== 'undefined') {
  window.addEventListener('DOMContentLoaded', () => {
    setTimeout(criarIndicadorConexao, 2000);
  });
}

// ================ BUSCA GLOBAL ================

function mostrarBuscaGlobal() {
  const html = `
    <div id="buscaGlobal" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); display: flex; align-items: flex-start; justify-content: center; z-index: 10002; padding-top: 100px; backdrop-filter: blur(4px);" onclick="if(event.target.id === 'buscaGlobal') this.remove()">
      <div style="background: var(--bg-card); border-radius: 12px; width: 100%; max-width: 700px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); overflow: hidden; animation: slideDown 0.2s ease-out;" onclick="event.stopPropagation()">
        
        <div style="padding: 20px; border-bottom: 1px solid var(--border);">
          <div style="position: relative;">
            <i class="fas fa-search" style="position: absolute; left: 16px; top: 50%; transform: translateY(-50%); color: var(--text-secondary); font-size: 18px;"></i>
            <input 
              type="text" 
              id="buscaGlobalInput" 
              placeholder="Buscar em produtos, notas, tags, histórico..." 
              autofocus
              oninput="executarBuscaGlobal(this.value)"
              onkeydown="if(event.key === 'Escape') document.getElementById('buscaGlobal').remove()"
              style="width: 100%; padding: 14px 16px 14px 52px; border: 2px solid var(--primary); border-radius: 8px; font-size: 16px; background: var(--bg-main); color: var(--text-primary); outline: none; font-family: inherit;"
            />
          </div>
        </div>
        
        <div id="resultadosBusca" style="max-height: 500px; overflow-y: auto; padding: 16px;">
          <div style="text-align: center; padding: 60px 20px; color: var(--text-secondary);">
            <i class="fas fa-search" style="font-size: 64px; opacity: 0.3; margin-bottom: 16px;"></i>
            <p>Digite para buscar em todo o sistema</p>
            <small>Produtos • Notas • Tags • Histórico</small>
          </div>
        </div>
        
        <div style="padding: 12px 20px; background: var(--bg-secondary); border-top: 1px solid var(--border); display: flex; gap: 20px; font-size: 11px; color: var(--text-secondary);">
          <span><kbd>Esc</kbd> Fechar</span>
...
function executarBuscaGlobal(termo) {
  ...
  timeoutBuscaGlobal = setTimeout(() => {
    ...
    // Buscar em produtos
    if (window.dadosEstoque && Array.isArray(window.dadosEstoque)) {
      resultados.produtos = window.dadosEstoque.filter(p => 
        p.nome?.toLowerCase().includes(termoLower) ||
        p.marca?.toLowerCase().includes(termoLower) ||
        p.codigo?.toLowerCase().includes(termoLower) ||
        p.lote?.toLowerCase().includes(termoLower)
      ).slice(0, 5);
    }
    
    // Buscar em tags
    resultados.tags = Array.from(tagsDisponiveis).filter(tag => 
      tag.toLowerCase().includes(termoLower)
    ).slice(0, 5);
    
    // Buscar em notas (simplificado - em produção seria mais complexo)
    produtos.forEach(produto => {
      const notas = obterNotasProduto(produto.id);
      const notasEncontradas = notas.filter(nota => 
        nota.texto.toLowerCase().includes(termoLower)
      );
      if (notasEncontradas.length > 0) {
        resultados.notas.push({ produto, notas: notasEncontradas.slice(0, 2) });
      }
    });
    resultados.notas = resultados.notas.slice(0, 3);
    
    const totalResultados = resultados.produtos.length + resultados.notas.length + resultados.tags.length;
    ...
    let html = `<div style="font-size: 12px; color: var(--text-secondary); margin-bottom: 16px; font-weight: 600;">${totalResultados} resultado(s) encontrado(s)</div>`;
      ...
      html += `
        <div style="margin-bottom: 24px;">
          <div style="font-size: 13px; font-weight: 600; color: var(--text-primary); margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
            <span style="font-size: 18px;">📦</span> Produtos (${resultados.produtos.length})
          </div>
          ${resultados.produtos.map(p => `
            <div onclick="abrir('estoque'); document.getElementById('buscaGlobal').remove();" style="background: var(--bg-secondary); border-radius: 8px; padding: 12px; margin-bottom: 8px; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.background='var(--primary)'; this.style.color='white';" onmouseout="this.style.background='var(--bg-secondary)'; this.style.color='inherit';">
              <div style="font-weight: 600; margin-bottom: 4px;">${p.nome}</div>
              <div style="font-size: 12px; opacity: 0.8;">${p.marca} • ${p.codigo || 'S/N'}</div>
            </div>
          `).join('')}
        </div>
      `;
    ...
    // Tags
    if (resultados.tags.length > 0) {
      html += `
        <div style="margin-bottom: 24px;">
          <div style="font-size: 13px; font-weight: 600; color: var(--text-primary); margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
            <span style="font-size: 18px;">🏷️</span> Tags (${resultados.tags.length})
          </div>
          <div style="display: flex; flex-wrap: wrap; gap: 8px;">
            ${resultados.tags.map(tag => `
              <span onclick="mostrarGerenciadorTags(); document.getElementById('buscaGlobal').remove();" style="display: inline-block; padding: 6px 12px; background: ${getCorTag(tag)}; color: white; border-radius: 6px; font-size: 13px; cursor: pointer; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">${tag}</span>
            `).join('')}
          </div>
        </div>
      `;
    ...
    // Notas
    if (resultados.notas.length > 0) {
      html += `
        <div style="margin-bottom: 24px;">
          <div style="font-size: 13px; font-weight: 600; color: var(--text-primary); margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
            <span style="font-size: 18px;">📝</span> Notas (${resultados.notas.length})
          </div>
          ${resultados.notas.map(item => `
            <div onclick="mostrarModalNotas('${item.produto.id}'); document.getElementById('buscaGlobal').remove();" style="background: var(--bg-secondary); border-radius: 8px; padding: 12px; margin-bottom: 8px; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.background='var(--primary)'; this.style.color='white';" onmouseout="this.style.background='var(--bg-secondary)'; this.style.color='inherit';">
              <div style="font-weight: 600; margin-bottom: 4px;">${item.produto.nome}</div>
              <div style="font-size: 12px; opacity: 0.8;">${item.notas[0].texto.substring(0, 60)}...</div>
            </div>
          `).join('')}
        </div>
      `;
...
// ========================================
// SISTEMA DE DASHBOARD PERSONALIZÁVEL
// ========================================

let dashboardConfig = JSON.parse(localStorage.getItem('dashboardConfig') || 
  '{"widgets":["metricas","graficos","notificacoes","atalhos"],"layout":"grade"}');

function abrirEditorDashboard() {
  const modal = document.createElement('div');
  ...
  const widgetsDisponiveis = [
    { id: 'metricas', nome: '📊 Métricas Rápidas', desc: 'Total produtos, vencidos, etc' },
    { id: 'graficos', nome: '📈 Gráficos', desc: '4 gráficos de análise' },
    { id: 'notificacoes', nome: '🔔 Notificações', desc: 'Alertas importantes' },
    { id: 'atalhos', nome: '⚡ Atalhos Rápidos', desc: 'Ações comuns' },
    { id: 'relatorio', nome: '📄 Relatório', desc: 'Resumo do dia' },
    { id: 'agenda', nome: '📅 Agenda', desc: 'Próximos vencimentos' }
  ];
  
  let widgetsHTML = widgetsDisponiveis.map(w => {
    const ativo = dashboardConfig.widgets.includes(w.id);
    return `
      <div class="form-check" style="padding: 12px; border: 1px solid #dee2e6; border-radius: 8px; margin-bottom: 8px; background: ${ativo ? '#e7f3ff' : '#fff'};">
        <input class="form-check-input" type="checkbox" id="widget-${w.id}" ${ativo ? 'checked' : ''} value="${w.id}">
        <label class="form-check-label" for="widget-${w.id}" style="cursor: pointer;">
          <strong>${w.nome}</strong><br>
          <small style="color: #6c757d;">${w.desc}</small>
        </label>
      </div>
    `;
  }).join('');
  ...
  modal.innerHTML = `
    <div class="modal-dialog modal-dialog-centered" style="max-width: 600px;">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <h6 style="margin-bottom: 16px;">Widgets Disponíveis</h6>
          <div id="listaWidgets">
            ${widgetsHTML}
          </div>
          
          <h6 style="margin-top: 24px; margin-bottom: 12px;">Layout</h6>
          <div class="btn-group w-100" role="group">
            <input type="radio" class="btn-check" name="layoutRadio" id="layoutGrade" value="grade" ${dashboardConfig.layout === 'grade' ? 'checked' : ''}>
            <label class="btn btn-outline-primary" for="layoutGrade">📱 Grade</label>
            
            <input type="radio" class="btn-check" name="layoutRadio" id="layoutLista" value="lista" ${dashboardConfig.layout === 'lista' ? 'checked' : ''}>
            <label class="btn btn-outline-primary" for="layoutLista">📋 Lista</label>
            
            <input type="radio" class="btn-check" name="layoutRadio" id="layoutCompacto" value="compacto" ${dashboardConfig.layout === 'compacto' ? 'checked' : ''}>
            <label class="btn btn-outline-primary" for="layoutCompacto">🗜️ Compacto</label>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" onclick="this.closest('.modal').remove()">Cancelar</button>
          <button class="btn btn-primary" onclick="salvarDashboardConfig()">
...
window.abrirEditorDashboard = abrirEditorDashboard;

function salvarDashboardConfig() {
  const widgets = Array.from(document.querySelectorAll('#listaWidgets input[type="checkbox"]:checked'))
    .map(cb => cb.value);
  const layout = document.querySelector('input[name="layoutRadio"]:checked').value;
  ...
  mostrarNotificacao('✅ Dashboard personalizado com sucesso!', 'success');
  
  // Recarregar dashboard se estiver aberto
  if (document.getElementById('dashboard').classList.contains('tela-ativa')) {
    mostrarTela('dashboard');
  }
}
window.salvarDashboardConfig = salvarDashboardConfig;

// ========================================
// SISTEMA DE MODO ESCURO AUTOMÁTICO
// ========================================

function inicializarDarkModeAutomatico() {
  // Verificar se é primeira visita
  if (!localStorage.getItem('temaEscolhido')) {
    // Detectar preferência do sistema
    if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
      aplicarTema('escuro');
      localStorage.setItem('temaEscolhido', 'escuro');
    }
  }
  ...
  if (window.matchMedia) {
    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', e => {
      if (!localStorage.getItem('temaManualmenteAlterado')) {
...
const passosTour = [
  {
    elemento: '#btnDashboard',
    titulo: '👋 Bem-vindo ao Sistema FEFO!',
    texto: 'Este é o seu Dashboard. Aqui você vê métricas e gráficos do estoque.',
    posicao: 'bottom'
  },
  {
    elemento: '#btnEstoque',
    titulo: '📦 Estoque',
    texto: 'Aqui você gerencia todos os produtos. Use os filtros e busca para encontrar produtos rapidamente.',
    posicao: 'bottom'
  },
  {
    elemento: '#btnTags',
    titulo: '🏷️ Tags',
    texto: 'Organize produtos com tags coloridas! Crie categorias personalizadas.',
    posicao: 'bottom'
  },
  {
    elemento: '#btnAnalytics',
    titulo: '📊 Analytics',
    texto: 'Acompanhe métricas de produtividade, ações realizadas e muito mais.',
    posicao: 'bottom'
  },
  {
    elemento: '#btnBackup',
    titulo: '💾 Backups',
    texto: 'Configure backups automáticos para nunca perder seus dados.',
    posicao: 'bottom'
  },
  {
    elemento: '#btnNotificacoes',
    titulo: '🔔 Notificações',
    texto: 'Receba alertas sobre produtos vencidos, estoque baixo e muito mais.',
    posicao: 'bottom'
  },
  {
    elemento: 'body',
    titulo: '⌨️ Atalhos de Teclado',
    texto: `Use atalhos para ser mais produtivo:
<div style="margin-top: 12px; text-align: left; background: #f8f9fa; padding: 12px; border-radius: 8px; font-family: monospace; font-size: 13px;">
<strong>Ctrl+P</strong> = Command Palette<br>
<strong>Ctrl+F</strong> = Busca Global<br>
<strong>Ctrl+K</strong> = Buscar produto<br>
<strong>Ctrl+N</strong> = Novo produto<br>
<strong>Ctrl+Z</strong> = Desfazer<br>
<strong>Ctrl+H</strong> = Ver atalhos
</div>`,
    posicao: 'center'
  },
  {
    elemento: 'body',
    titulo: '🎉 Pronto!',
    texto: 'Você está pronto para usar o sistema! Pressione <kbd>Ctrl+P</kbd> a qualquer momento para ver todos os comandos disponíveis.',
    posicao: 'center'
  }
];
...
function mostrarPassoTour() {
  ...
  const overlayAnterior = document.getElementById('tourOverlay');
  if (overlayAnterior) overlayAnterior.remove();
  
  if (passoTourAtual >= passosTour.length) {
    localStorage.setItem('tourConcluido', 'true');
    mostrarNotificacao('✅ Tour concluído! Aproveite o sistema!', 'success');
    return;
  }
  
  const passo = passosTour[passoTourAtual];
  ...
  if (passo.elemento !== 'body') {
    elementoAlvo = document.querySelector(passo.elemento);
    if (elementoAlvo) {
      ...
      if (passo.posicao === 'bottom') {
        tooltipStyle = `top: ${rect.bottom + 20}px; left: ${rect.left}px;`;
      } else if (passo.posicao === 'top') {
        tooltipStyle = `bottom: ${window.innerHeight - rect.top + 20}px; left: ${rect.left}px;`;
      }
    }
  }
  ...
  tooltip.innerHTML = `
    <div style="margin-bottom: 16px;">
      <h5 style="margin: 0 0 8px 0; color: #667eea;">${passo.titulo}</h5>
      <p style="margin: 0; color: #495057; line-height: 1.6;">${passo.texto}</p>
    </div>
    <div style="display: flex; justify-content: space-between; align-items: center;">
      <small style="color: #6c757d;">Passo ${passoTourAtual + 1} de ${passosTour.length}</small>
      <div>
        ${passoTourAtual > 0 ? '<button class="btn btn-sm btn-outline-secondary" onclick="voltarPassoTour()" style="margin-right: 8px;">← Voltar</button>' : ''}
        <button class="btn btn-sm btn-secondary" onclick="pularTour()" style="margin-right: 8px;">Pular</button>
        <button class="btn btn-sm btn-primary" onclick="proximoPassoTour()">
          ${passoTourAtual === passosTour.length - 1 ? 'Concluir' : 'Próximo →'}
        </button>
      </div>
    </div>
  `;
...
window.proximoPassoTour = proximoPassoTour;

function voltarPassoTour() {
  passoTourAtual--;
  mostrarPassoTour();
}
window.voltarPassoTour = voltarPassoTour;

function pularTour() {
  document.getElementById('tourOverlay').remove();
  localStorage.setItem('tourConcluido', 'pular');
}
window.pularTour = pularTour;

// Verificar se deve mostrar tour na primeira visita
if (!localStorage.getItem('tourConcluido')) {
  setTimeout(() => {
    if (auth.currentUser) {
      iniciarTour();
    }
...
// ========================================
// BARRA LATERAL DE AÇÕES RÁPIDAS
// ========================================

let barraSidebarAberta = false;
let acoesRapidas = JSON.parse(localStorage.getItem('acoesRapidas') || 
  '["novoProduto","buscar","importar","exportar","notificacoes","analytics"]');

const acoesDisponiveis = {
  novoProduto: { icon: '➕', nome: 'Novo Produto', acao: () => { abrir('estoque'); document.getElementById('nomeProduto')?.focus(); } },
  buscar: { icon: '🔍', nome: 'Buscar', acao: () => { abrir('estoque'); document.getElementById('buscaEstoque')?.focus(); } },
  importar: { icon: '📥', nome: 'Importar', acao: () => document.getElementById('fileImport')?.click() },
  exportar: { icon: '📤', nome: 'Exportar', acao: () => exportarExcel() },
  notificacoes: { icon: '🔔', nome: 'Notificações', acao: () => mostrarCentroNotificacoes() },
  analytics: { icon: '📊', nome: 'Analytics', acao: () => mostrarAnalytics() },
  backup: { icon: '💾', nome: 'Backup', acao: () => mostrarGerenciadorBackups() },
  temas: { icon: '🎨', nome: 'Temas', acao: () => mostrarSeletorTemas() },
  commandPalette: { icon: '⌨️', nome: 'Comandos', acao: () => mostrarCommandPalette() },
  buscarGlobal: { icon: '🔍', nome: 'Busca Global', acao: () => mostrarBuscaGlobal() },
  tags: { icon: '🏷️', nome: 'Tags', acao: () => mostrarGerenciadorTags() }
};

function criarBarraSidebar() {
  ...
  `;
  fab.innerHTML = '⚡';
  fab.onclick = toggleBarraSidebar;
  
  fab.onmouseenter = () => {
    fab.style.transform = 'scale(1.1) rotate(5deg)';
    fab.style.boxShadow = '0 6px 30px rgba(102, 126, 234, 0.6)';
  };
  fab.onmouseleave = () => {
  ...
  sidebar.id = 'quickActionsSidebar';
  ...
  sidebar.innerHTML = `
    <div style="padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; display: flex; justify-content: space-between; align-items: center;">
      <h5 style="margin: 0;">⚡ Ações Rápidas</h5>
      <button onclick="toggleBarraSidebar()" style="background: none; border: none; color: white; font-size: 24px; cursor: pointer; padding: 0; width: 32px; height: 32px;">×</button>
    </div>
    
    <div style="flex: 1; overflow-y: auto; padding: 16px;">
      <div id="listaAcoesRapidas"></div>
    </div>
    
    <div style="padding: 16px; border-top: 1px solid var(--border); background: var(--bg-secondary);">
      <button class="btn btn-sm btn-outline-primary w-100" onclick="configurarAcoesRapidas()">
...
function toggleBarraSidebar() {
  barraSidebarAberta = !barraSidebarAberta;
  const sidebar = document.getElementById('quickActionsSidebar');
  const fab = document.getElementById('sidebarFAB');
  
  if (barraSidebarAberta) {
    sidebar.style.right = '0';
    fab.style.right = '300px';
    fab.innerHTML = '×';
  } else {
...
window.toggleBarraSidebar = toggleBarraSidebar;

function renderizarAcoesRapidas() {
  const lista = document.getElementById('listaAcoesRapidas');
  if (!lista) return;
  
  lista.innerHTML = acoesRapidas.map((acaoId, idx) => {
    const acao = acoesDisponiveis[acaoId];
    if (!acao) return '';
    
    return `
      <div 
        class="quick-action-item"
        onclick="executarAcaoRapida('${acaoId}')"
        style="
          padding: 16px;
          margin-bottom: 8px;
          ...
          transition: all 0.2s;
        "
        onmouseover="this.style.transform='translateX(-4px)'; this.style.boxShadow='0 4px 12px rgba(0,0,0,0.1)'; this.style.borderColor='var(--primary)';"
        onmouseout="this.style.transform='translateX(0)'; this.style.boxShadow='none'; this.style.borderColor='var(--border)';"
      >
        <span style="font-size: 24px;">${acao.icon}</span>
        <span style="flex: 1; font-weight: 500; color: var(--text-primary);">${acao.nome}</span>
      </div>
    `;
  }).join('');
}
window.renderizarAcoesRapidas = renderizarAcoesRapidas;

function executarAcaoRapida(acaoId) {
  const acao = acoesDisponiveis[acaoId];
  if (acao && acao.acao) {
    acao.acao();
    registrarAcao('acaoRapida', { nome: acao.nome });
    toggleBarraSidebar();
  }
}
window.executarAcaoRapida = executarAcaoRapida;
...
function configurarAcoesRapidas() {
  ...
  const listaHTML = Object.entries(acoesDisponiveis).map(([id, acao]) => {
    const ativo = acoesRapidas.includes(id);
    return `
      <div class="form-check" style="padding: 12px; border: 1px solid #dee2e6; border-radius: 8px; margin-bottom: 8px; background: ${ativo ? '#e7f3ff' : '#fff'};">
        <input class="form-check-input" type="checkbox" id="acao-${id}" ${ativo ? 'checked' : ''} value="${id}">
        <label class="form-check-label" for="acao-${id}" style="cursor: pointer;">
          <span style="font-size: 20px; margin-right: 8px;">${acao.icon}</span>
  ...
  modal.innerHTML = `
    <div class="modal-dialog modal-dialog-centered" style="max-width: 500px;">
      <div class="modal-content">
        <div class="modal-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
          <h5 class="modal-title">⚙️ Configurar Ações Rápidas</h5>
          <button type="button" class="btn-close btn-close-white" onclick="this.closest('.modal').remove()"></button>
        </div>
        <div class="modal-body">
          <p style="color: #6c757d; margin-bottom: 16px;">Selecione as ações que deseja ter na barra lateral:</p>
          <div id="listaConfiguracaoAcoes">
            ${listaHTML}
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" onclick="this.closest('.modal').remove()">Cancelar</button>
          <button class="btn btn-primary" onclick="salvarConfiguracaoAcoes()">
            <i class="fas fa-save"></i> Salvar
          </button>
        </div>
      </div>
...
window.configurarAcoesRapidas = configurarAcoesRapidas;

function salvarConfiguracaoAcoes() {
  const checkboxes = document.querySelectorAll('#listaConfiguracaoAcoes input[type="checkbox"]:checked');
  acoesRapidas = Array.from(checkboxes).map(cb => cb.value);
  localStorage.setItem('acoesRapidas', JSON.stringify(acoesRapidas));
  
  renderizarAcoesRapidas();
  document.querySelector('.modal').remove();
  mostrarNotificacao('✅ Ações rápidas configuradas!', 'success');
}
window.salvarConfiguracaoAcoes = salvarConfiguracaoAcoes;

// Inicializar barra lateral quando usuário estiver logado
setTimeout(() => {
  if (auth.currentUser && !document.getElementById('sidebarFAB')) {
    criarBarraSidebar();
  }
}, 1000);
...
function autoSalvarRascunho() {
  ...
  autoSaveTimeout = setTimeout(() => {
    const rascunho = {
      ...
      lote: document.getElementById('loteProduto')?.value,
    ...
    if (rascunho.nome || rascunho.codigo) {
      localStorage.setItem('rascunho-produto', JSON.stringify(rascunho));
  ...
  try {
    ...
    if (idade < 30 * 60 * 1000) {
      if (confirm('📝 Encontramos um rascunho não salvo. Deseja recuperar?')) {
        ...
        mostrarToast('✅ Rascunho recuperado!', 'success');
      } else {
        localStorage.removeItem('rascunho-produto');
      }
    } else {
...
function limparCacheAntigo() {
  try {
    ...
    chaves.forEach(chave => {
      if (chave.startsWith('cache-') || chave.startsWith('backup-')) {
        try {
          const dados = JSON.parse(localStorage.getItem(chave));
          if (dados.timestamp && agora - dados.timestamp > 24 * 60 * 60 * 1000) {
    ...
    if (limpezas > 0) {
      console.log(`🧹 ${limpezas} cache(s) antigo(s) limpo(s)`);
    }
  } catch (e) {
...
if (typeof window !== 'undefined') {
  window.addEventListener('DOMContentLoaded', () => {
    const inputIds = ['codigoBarras', 'nomeProduto', 'marcaProduto', 'loteProduto', 
                      'fornecedorProduto', 'quantidadeProduto', 'estoqueMinimo', 'validadeProduto'];
    
    inputIds.forEach(id => {
      const input = document.getElementById(id);
      if (input) {
        input.addEventListener('input', autoSalvarRascunho);
      }
    });
...
// Expor funções globalmente
window.verificarLimiteUsoPorPlano = verificarLimiteUsoPorPlano;
window.verificarProdutoDuplicado = verificarProdutoDuplicado;
window.calcularDiasVencimento = calcularDiasVencimento;
window.detectarDispositivo = detectarDispositivo;
window.autoSalvarRascunho = autoSalvarRascunho;
